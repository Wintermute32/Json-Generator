	...
	public class NewRootGeneration
	{
		public NewRoot GenerateNewRoot(Form1 form)
		{
			...
			NewRoot finalRoot = new NewRoot()
			{
				...
				appearance = new Appearance(form),
		...
		{
			var popLists = flowList.Find(x => x.Name == "featuredPopPanel").Controls.OfType<ComboBox>().ToList();
		...
		public static List<LastChanceBoxPrize> AssignBoxValues(Dictionary<string, string> popDict)
		{
			...
			string[] rarityOrder = new string[] { "common", "rare", "epic", "legendary", "event exclusive" };

				for (int i = 0; i < rarityOrder.Length; i++)
					foreach (var x in popDict)
						if (rarityOrder[i] == x.Value)
						{
			...
			for (int i = 0; i < lcbpList.Count; i++)
			{
				...
				{
					case 0:
						lcbpList[i].amount = 1; lcbpList[i].instances = 6; break;
					case 1:
						lcbpList[i].amount = 2; lcbpList[i].instances = 4; break;
					case 2:
						lcbpList[i].amount = 3; lcbpList[i].instances = 3; break;
					case 3:
						lcbpList[i].amount = 4; lcbpList[i].instances = 2; break;
					case 4:
						lcbpList[i].amount = 5; lcbpList[i].instances = 1; break;
				}
			}
			return lcbpList;
		}
